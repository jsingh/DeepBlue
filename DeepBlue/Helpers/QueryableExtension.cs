...
namespace DeepBlue.Helpers {
	...
	public static class QueryableExtension {
		...
		public static IQueryable<T> EntityFilter<T>(this IQueryable<T> source) {
			var type = typeof(T);
			...
			if (isEntityColumnExist) {
				...
				if (permission != null) {
					...
					if (permission.IsSystemEntity == true && permission.IsOtherEntity == false)
						expression = string.Format("EntityID={0}", (int)ConfigUtil.SystemEntityID);

					if (permission.IsSystemEntity == false && permission.IsOtherEntity == true)
						expression = string.Format("EntityID={0}", Authentication.CurrentEntity.EntityID);

					if (permission.IsSystemEntity && permission.IsOtherEntity)
						expression = string.Format("(EntityID={0} || EntityID={1})", (int)ConfigUtil.SystemEntityID, Authentication.CurrentEntity.EntityID);

					if (permission.IsSystemEntity == false && permission.IsOtherEntity == false)
						expression = "EntityID<0";
				}
				else {
					expression = string.Format("EntityID={0}", Authentication.CurrentEntity.EntityID);
				}
				source = (IQueryable<T>)source.Where<T>(expression);
			}
			return source;
		}
