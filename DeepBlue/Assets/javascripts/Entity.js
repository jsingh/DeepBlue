var entity={
	add: function (that) {
		...
		if(!row) {
			...
			var data={ "page": 0,"total": 0,"rows": [{ "cell": [0,"","",true]}] };
	...
	,deleteRow: function (img,id) {
		if(confirm("Are you sure you want to delete this entity?")) {
			...
			img.src=imgsrc;
			var dt=new Date();
			var url="/Admin/DeleteEntity/"+id+"?t="+dt.getTime();
	...
	,save: function (img,id) {
		...
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditEntity?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
