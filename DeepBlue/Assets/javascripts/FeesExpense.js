var feesExpenseReport={
	...
	,onSubmit: function (frm) {
		try {
			...
			target.show();
			var grid=$("#FeesExpenseList");
			grid.flexRemoveSortClass();
			grid.flexOptions({ sortname: "" });
			feesExpenseReport.onGridInit();
	...
	,onGridInit: function () {
		var t=$("#FeesExpenseList");
		$("thead th:eq(0)").addClass("sorted");
		$("thead th:eq(0) div span").addClass("sdesc");
	...
	,exportData: function () {
		...
		if(exportTypeId=="1"||exportTypeId=="4") {
			...
			url="/Report/ExportFeesExpenseDetail?FundId="+$("#FundId").val()+"&ExportTypeId="+exportTypeId;
