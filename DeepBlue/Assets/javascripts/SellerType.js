var sellerType={
	...
	,deleteRow: function (img,id) {
		if(confirm("Are you sure you want to delete this seller type?")) {
			var tr=$(img).parents("tr:first");
			var imgsrc=img.src;
			$(img).attr("src","/Assets/images/ajax.jpg");
			img.src=imgsrc;
			var dt=new Date();
			var url="/Admin/DeleteSellerType/"+id+"?t="+dt.getTime();
	...
	,save: function (img,id) {
		...
		var url="/Admin/UpdateSellerType";
		var imgsrc=img.src;
		$(img).attr("src","/Assets/images/ajax.jpg");
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditSellerType?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
