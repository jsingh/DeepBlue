var underlyingFundType={
	...
	,deleteRow: function (img,id) {
		if(confirm("Are you sure you want to delete this underlying fund type?")) {
			var tr=$(img).parents("tr:first");
			var imgsrc=img.src;
	...
	,save: function (img,id) {
		...
		var url="/Admin/UpdateUnderlyingFundType";
		var imgsrc=img.src;
		$(img).attr("src","/Assets/images/ajax.jpg");
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditUnderlyingFundType?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
	...
	,onInit: function (g) {
		var data={ name: "Add Underlying Fund Type" };
		$("#AddButtonTemplate").tmpl(data).prependTo(g.pDiv);
		//		$(window).resize(function () {
		//			underlyingFundType.resizeGV(g);
		//		});
	}
