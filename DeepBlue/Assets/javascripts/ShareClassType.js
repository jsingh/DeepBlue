var shareClassType={
	...
	,save: function (img,id) {
		...
		var url="/Admin/UpdateShareClassType";
		var imgsrc=img.src;
		$(img).attr("src","/Assets/images/ajax.jpg");
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditShareClassType?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
	...
	,onInit: function (g) {
		var data={ name: "Add Share Class Type" };
		$("#AddButtonTemplate").tmpl(data).prependTo(g.pDiv);
		//		$(window).resize(function () {
		//			shareClassType.resizeGV(g);
		//		});
	}
