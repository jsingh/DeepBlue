var reportingFrequency={
	...
	,init: function () {
		$("document").ready(function () {
			reportingFrequency.resizeIframe();
			reportingFrequency.pageLoad=true;
	...
	,add: function (id) {
		...
		$("#addReportingFrequencyDialog").remove();
		var iframe=document.createElement("div");
		iframe.id="addReportingFrequencyDialog";
	...
	,onRowClick: function (row) {
		reportingFrequency.add(row.cell[0]);
	}
	,closeDialog: function (reload) {
		$("#addReportingFrequencyDialog").dialog('close');
		if(reload==true) {
			$("#ReportingFrequencyList").flexReload();
		}
	}
