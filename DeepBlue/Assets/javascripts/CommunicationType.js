var communicationType={
	...
	,deleteRow: function (img,id) {
		if(confirm("Are you sure you want to delete this communication entity type?")) {
			var tr=$(img).parents("tr:first");
			var imgsrc=img.src;
	...
	,save: function (img,id) {
		...
		var url="/Admin/UpdateCommunicationType";
		var imgsrc=img.src;
		$(img).attr("src","/Assets/images/ajax.jpg");
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditCommunicationType?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
