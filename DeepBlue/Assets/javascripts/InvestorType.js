var investorType={
	...
	,save: function (img,id) {
		...
		var url="/Admin/UpdateInvestorType";
		var imgsrc=img.src;
		$(img).attr("src","/Assets/images/ajax.jpg");
		$.post(url,param,function (data) {
			...
			if(arr[0]!="True") {
				...
				$.getJSON("/Admin/EditInvestorType?_"+(new Date).getTime()+"&id="+arr[1],function (loadData) {
	...
	,onInit: function (g) {
		var data={ name: "Add Investor Type" };
		$("#AddButtonTemplate").tmpl(data).prependTo(g.pDiv);
//		$(window).resize(function () {
//			investorType.resizeGV(g);
//		});
	}
