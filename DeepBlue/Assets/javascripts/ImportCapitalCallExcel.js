...
var importCapitalCallExcel={
			...
			$.ajaxFileUpload(
			{
				...
				success: function(data,status) {
					...
					if($.trim(data.Result)!="") {
						...
						importCapitalCallExcel.importRows(data.FilePath,data.FileName);
					}
				},
	...
	,selectExcelTab: function(ddl) {
		var section=$(ddl).parents(".capitalCallimportsection:first");
		...
		$.each(importCapitalCallExcel.lastExcelData.Tables,function(i,item) {
	...
	,importRows: function(filePath,fileName) {
		...
		$.post("/CapitalCall/ImportExcel",params,function(data) {
			importCapitalCallExcel.lastExcelData=null;
			target.css({
				"width": "auto",
				"height": "auto"
			});
			if($.trim(data.Result)!="") {
				...
				importCapitalCallExcel.lastExcelData=data;
	...
	,import: function(btn) {
		$(btn).hide();
		importCapitalCallExcel.importCapitalCall();
	}
	...
	,importCapitalCall: function() {
		$("#CapitalCallDetailTab").click();
		var thread=new ImportExcel();
		thread.box=$("#CapitalCallDetailBox");
		thread.url="/CapitalCall/ImportCapitalCallExcel";
		thread.onComplete = function (sessionKey, tableName, data) {
			if ($.trim(tableName) != "" && data.ErrorRows > 0) {
				...
				$("a",spnerrorexcel).click(function() {
					importCapitalCallExcel.expertErrorExcel(sessionKey,tableName);
				});
			}
		}
		thread.import();
