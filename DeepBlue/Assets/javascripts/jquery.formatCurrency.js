...
if(settings.checkZero){if(parseFloat(num)==0){$destination[$destination.is('input, select, textarea')?'val':'html']("");}}});};$.fn.toNumber=function(settings){var defaults=$.extend({name:"toNumber",region:'',global:true},$.formatCurrency.regions['']);settings=jQuery.extend(defaults,settings);if(settings.region.length>0){settings=$.extend(settings,getRegionOrCulture(settings.region));}
