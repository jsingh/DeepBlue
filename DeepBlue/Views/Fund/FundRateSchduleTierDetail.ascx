...
<td>
		<%: Html.jQueryTemplateTextBox("${index}_${rowIndex}_Rate", "${checkNullOrZero(tier.Rate)}", new { @id = "Rate", @disabled = "disabled", @onkeydown = "return jHelper.isCurrency(event);", @onchange = "javascript:fund.changeRate(this);fund.checkChange(this);" })%>
</td>
<td>
		<%: Html.jQueryTemplateTextBox("${index}_${rowIndex}_FlatFee", "${checkNullOrZero(tier.FlatFee)}", new { @id = "FlatFee", @disabled = "disabled", @onkeydown = "return jHelper.isCurrency(event);", @onchange = "javascript:fund.checkChange(this);" })%>
</td>
<td>
