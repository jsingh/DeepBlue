...
<td>
		{{if rowIndex==1}}
		<%: Html.jQueryTemplateTextBox("${index}_${rowIndex}_StartDate", "${formatDate(tier.StartDate)}" , new { @class="datefield", @style="width:80px", @inputname="StartDate",  @onchange = "javascript:fund.dateChecking(this);fund.checkChange(this);" })%>
		{{else}}
...
<td>
		<%: Html.jQueryTemplateTextBox("${index}_${rowIndex}_Rate", "${checkNullOrZero(tier.Rate)}", new { @id = "Rate", @style = "width:100px", @disabled = "disabled", @onkeydown = "return jHelper.isCurrency(event);", @onchange = "javascript:fund.changeRate(this);fund.checkChange(this);" })%>
</td>
<td>
		<%: Html.jQueryTemplateTextBox("${index}_${rowIndex}_FlatFee", "${checkNullOrZero(tier.FlatFee)}", new { @id = "FlatFee", @style = "width:100px", @disabled = "disabled", @onkeydown = "return jHelper.isCurrency(event);", @onchange = "javascript:fund.checkChange(this);" })%>
</td>
<td>
