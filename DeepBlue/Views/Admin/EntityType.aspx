...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
	<div class="admin-main">
		<div class="admin-content">
			...
			<table cellpadding="0" cellspacing="0" border="0" id="InvEntityTypeList" class="grid">
				<thead>
					<tr>
						<th sortname="InvestorEntityTypeName" style="width: 40%">
...
<tr id="Row${row.cell[0]}" {{if i%2>0}}class="erow"{{else}}class="grow"{{/if}}>
	<td style="width: 40%">
		...
		<%: Html.TextBox("InvestorEntityTypeName", "${row.cell[1]}", new { @class = "hide" })%>
	</td>
	...
	<td style="text-align:right;">
		{{if row.cell[0]==0}}
		<%: Html.Image("add_active.png", new { @id = "Add", @style="display:none;cursor:pointer;" , @onclick = "javascript:invEntityType.save(this,${row.cell[0]});" })%>
		{{else}}
		<%: Html.Image("Save_active.png", new { @id = "Save", @style="display:none;cursor:pointer;", @onclick = "javascript:invEntityType.save(this,${row.cell[0]});" })%>
		<%: Html.Image("Edit.png", new { @class = "gbutton editbtn show", @onclick = "javascript:invEntityType.edit(this);" })%>
		<%: Html.Image("largedel.png", new { @class = "gbutton show", @onclick = "javascript:invEntityType.deleteRow(this,${row.cell[0]});" })%>
		{{/if}}
		<%: Html.Hidden("InvestorEntityTypeId", "${row.cell[0]}") %>
	</td>
</tr>
{{/each}}
	</script>
