...
<% flexData.total = Convert.ToInt32(ViewData["TotalRows"]);
   ...
   foreach (var item in Model) {
	   ...
	   row.cell.Add(Html.Image("Edit.gif", new { @style = "cursor:pointer", @onclick = "javascript:invEntityType.add(" + item.InvestorEntityTypeID.ToString() + ");" }).ToHtmlString() + "&nbsp;&nbsp;&nbsp;" +
					Html.Image("Delete.png", new { @style = "cursor:pointer", @onclick = "javascript:invEntityType.deleteEntityType(" + item.InvestorEntityTypeID.ToString() + ",this);" }).ToHtmlString());
