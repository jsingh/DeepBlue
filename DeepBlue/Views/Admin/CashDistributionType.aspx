			...
			<table cellpadding="0" cellspacing="0" border="0" id="CashDistributionTypeList">
				<thead>
					<tr>
						...
						<th datatype="Boolean" sortname="Enabled" style="width: 10%;">
							Enable
						</th>
						<th>
						</th>
...
<tr id="Row${row.cell[0]}" {{if i%2>0}}class="erow"{{/if}}>
	...
	<td style="text-align:right;">
		{{if row.cell[0]==0}}
		<%: Html.Image("Add.png", new { @id = "Add", @style="display:none;cursor:pointer;" , @onclick = "javascript:cashDistributionType.save(this,${row.cell[0]});" })%>
		{{else}}
		<%: Html.Image("Save.png", new { @id = "Save", @style="display:none;cursor:pointer;", @onclick = "javascript:cashDistributionType.save(this,${row.cell[0]});" })%>
		<%: Html.Image("Edit.png", new { @class = "gbutton show", @onclick = "javascript:cashDistributionType.edit(this);" })%>
