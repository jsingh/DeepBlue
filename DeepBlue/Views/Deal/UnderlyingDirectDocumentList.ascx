<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<List<DeepBlue.Models.Deal.UnderlyingDirectDocumentList>>" %>
...
<% flexData.total=Convert.ToInt32(ViewData["TotalRows"]);
   ...
   foreach(var item in Model) {
	   ...
	   if(item.FilePath.ToLower().StartsWith("http://")||item.FilePath.ToLower().StartsWith("https://")) {
	   ...
	   row.cell.Add(Html.Anchor(Html.Image(imgname).ToHtmlString(),href,new { @class="gbutton",@target="_blank" }).ToHtmlString()+"&nbsp;&nbsp;"+
		   Html.Image("largedel.png",new { @style="cursor:pointer;",@class="gbutton",@onclick="javascript:dealDirect.deleteDocument("+item.DocumentId+",this);" }).ToHtmlString());
