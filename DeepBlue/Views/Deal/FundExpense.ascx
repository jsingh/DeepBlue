...
<tr id="FLE_${FundExpenseId}">
	...
	<td class="ralign">
		...
		{{if FundExpenseId>0}}
		<%: Html.Image("Save_active.png", new { @id = "save", @class = "default-button {{if FundExpenseId>0}}hide{{/if}}", @onclick = "javascript:dealActivity.addFLE(this,${FundExpenseId});" })%>
		<%: Html.Image("Edit.png", new { @class = "default-button show  gbutton", @onclick = "javascript:dealActivity.editFLE(this,${FundExpenseId});" })%>
		{{else}}
		<%: Html.Image("add_active.png", new { @id = "add", @class = "default-button", @onclick = "javascript:dealActivity.addFLE(this,0);" })%>
		{{/if}}
	</td>
</tr>
